...
bool CCalibration::writeParams()
{
    ...
    out<<camK.at<float>(2, 0)<<" "<<camK.at<float>(2, 1) <<" "<<camK.at<float>(2, 2) <<" "<<endl;
    out << endl;
#ifdef CV
	out << camDiscoeff.at<float>(0, 0) << " ";
	out << camDiscoeff.at<float>(0, 1) << " ";
	out << camDiscoeff.at<float>(0, 2) << " ";
	out << camDiscoeff.at<float>(0, 3) << " ";
	out << camDiscoeff.at<float>(0, 4) << endl;
#elif defined FISHEYE
	out << camDiscoeff.at<float>(0, 0) << " ";
	out << camDiscoeff.at<float>(0, 1) << " ";
...
bool CCalibration::readPatternImg(string preName="left")
{
    int imgNum=0;
    Mat img;
    do
    {
        ...
        ss<<imgNum;
#if !IMG_JPEG
        string path=patternImgPath + ss.str()+".bmp";
#else
		string path=patternImgPath + ss.str()+".jpeg";
#endif
        img=imread(path, 0);
        if (!img.data)
        {
...
void CCalibration::calibProcess()
{
    ...
    /**************************摄像机标定******************************/
    Size squre_size=Size(25,25);//棋盘格尺寸
...
void CCalibration::run(string preName="left")
{
    bool readSuccess=readPatternImg(preName);
	if (!readSuccess)
	{
